  ...
  RedisTweetCounts = (function() {
    ...
    function RedisTweetCounts(redis, precision) {
      ...
      this.regions = function(callback) {
        return RedisTweetCounts.prototype.regions.apply(_this, arguments);
      };
    ...
    RedisTweetCounts.prototype.regions = function(callback) {
      var _this = this;
      return this.redis.zrange(["" + this.version + ".geohashes:" + this.precision, 0, -1], function(err, response) {
        var fullId, geohashes, keyIndex;
        geohashes = (function() {
          var _i, _ref, _results;
          _results = [];
          for (keyIndex = _i = 0, _ref = response.length; 0 <= _ref ? _i < _ref : _i > _ref; keyIndex = 0 <= _ref ? ++_i : --_i) {
            fullId = response[keyIndex];
            _results.push(fullId.toString().substring(this.prefix.length));
          }
          return _results;
        }).call(_this);
        return callback(geohashes);
      });
    };
